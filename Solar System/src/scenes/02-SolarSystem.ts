...
export default class SolarSystemScene extends Scene {
    ...
    private drawSystem(parent: mat4, system: SolarSystemDescription){
        ...
        let wtSelf = system.rotationSpeedAroundSelf*this.time;
        mat4.rotateY(matPlanet, matPlanet, wtSelf);
        ...
        if(system.children)
        {
            ...
            for(let i = 0; i<system.children.length; ++i) {
                let child = mat4.clone(parent);  //clone the parent matrix
                let wtParent = system.children[i].rotationSpeedAroundParent*this.time;  //wt
                let r = system.children[i].distanceFromParent; //x = rcos(wt), y = 0 (rotation axis), z = + or - r sin(wt) (depends on the orbiting direction)

                //orbit around the parent
                mat4.translate(child, child, [r*Math.cos(wtParent), 0, -1*r*Math.sin(wtParent)]);

                //Draw the system and its children if exist
                this.drawSystem(child, system.children[i]);
            }
        }
    }
